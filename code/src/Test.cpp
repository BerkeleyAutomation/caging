...
Test::Test(const b2Vec2& gravity)
{
	init(gravity);
}

void
Test::init(const b2Vec2& gravity) {
...
void Test::Step(Settings* settings)
{
	...
	m_debugDraw.DrawString(5, m_textLine, "num contacts = %d",m_pointCount);
	m_textLine += DRAW_STRING_NEW_LINE;
	...
	if (settings->drawContactPoints)
	{
		...
		for (int32 i = 0; i < m_pointCount; ++i)
		{
			...
			if (settings->drawContactNormals == 1)
			{
				...
				b2Vec2 p2 = p1 + k_axisScale * 10 * point->normal;
