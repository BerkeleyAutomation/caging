                       ...
                       ccd_pt_t *polytope, ccd_pt_el_t **nearest)
{
    ...
    int ret, size;
    ccd_real_t dist = CCD_REAL_MAX;
    ccd_real_t prev_dist = CCD_REAL_MAX;
    ...
    *nearest = NULL;
    ...
    while (1){
        ...
        *nearest = ccdPtNearest(polytope);
        ...
        // terminate if no progress made (prevent infinite loops)
        if (k >= ccd->max_iterations || fabs(dist - prev_dist) < ccd->dist_tolerance) {
        ...
        /* printf("Dist %f\n", (*nearest)->dist); */
        /* printf("Type %d\n", (*nearest)->type); */
        /* printf("Wit %d %f %f %f\n", k, (*nearest)->witness.v[0], (*nearest)->witness.v[1], (*nearest)->witness.v[2]); */

        // get next support point
        int retval = nextSupport(obj1, obj2, ccd, *nearest, &supp);
        if (retval != 0) {
            break;
        }
    ...
    // only return valid if finished in given number of iterations
    return 0;
}
                              ...
                              ccd_pt_t *pt, ccd_pt_el_t **nearest)
{
    ...
    use_polytope3 = 0;
    dist = ccdVec3PointTriDist2(ccd_vec3_origin, &a->v, &b->v, &c->v, NULL);
    /* printf("Convert Dist %f\n", dist); */
    /* printf("Orig %f %f %f\n", ccd_vec3_origin->v[0], ccd_vec3_origin->v[1], ccd_vec3_origin->v[2]); */
    /* printf("A %f %f %f\n", a->v.v[0], a->v.v[1], a->v.v[2]); */
    /* printf("B %f %f %f\n", b->v.v[0], b->v.v[1], b->v.v[2]); */
    /* printf("C %f %f %f\n", c->v.v[0], c->v.v[1], c->v.v[2]); */
    /* printf("D %f %f %f\n", d->v.v[0], d->v.v[1], d->v.v[2]); */
    if (ccdIsZero(dist)){
        use_polytope3 = 1;
    }
    dist = ccdVec3PointTriDist2(ccd_vec3_origin, &a->v, &c->v, &d->v, NULL);
                       ...
                       ccd_support_t *out)
{
    ...
    //printf("Supp %f %f %f\n", out->v.v[0], out->v.v[1], out->v.v[2]);
    //printf("El %f %f %f %f %d\n", el->witness.v[0], el->witness.v[1], el->witness.v[2], el->dist, el->type);
